...
describe('Authentication & Authorization', () => {
  ...
  describe('Groups API Authorization', () => {
    it('should allow tenant admin to access their tenant groups', async () => {
      const token = createTestToken({
        sub: 'test@example.com',
        roles: ['tenant_admin'],
        tenantId: null,
      ...
      const res = await fastifyApp.inject({
        method: 'GET',
        url: `/tenants/${TEST_TENANT_ID}/groups`,
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
    ...
    it('should deny tenant admin access to other tenant groups', async () => {
      const token = createTestToken({
        sub: 'test@example.com',
        roles: ['tenant_admin'],
        tenantId: TEST_TENANT_ID,
      ...
      const res = await fastifyApp.inject({
        method: 'GET',
        url: '/tenants/other-tenant/groups',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
    ...
    it('should allow superadmin to access any tenant groups', async () => {
      const token = createTestToken({
    ...
    it('should deny unauthenticated access', async () => {
      ...
      expect(res.statusCode).toBe(401); // 401 Unauthorized is correct for missing auth header
    });
  });
